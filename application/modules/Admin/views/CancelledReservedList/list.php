...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <h1><?php echo lang("cancelled_reserved_list"); ?></h1>
        <ol class="breadcrumb">
            <li><a href="<?= base_url($this->type . '/dashboard') ?>"><i class="fa fa-dashboard"></i><?php echo lang('home'); ?></a></li>
            <li class="active"><?php echo lang("cancelled_reserved_list"); ?></li>
        </ol>
    </section>
    <!-- Main content -->
                        ...
                        <div id="example1_wrapper" class="dataTables_wrapper dt-bootstrap">
                          ...
                          <div class="form-group">
                            <input type="text" name="searchtext" id="searchtext" class="form-control input-sm" placeholder="Search" aria-controls="example1" value=""<?= !empty($searchtext) ? $searchtext : '' ?> >
												...
												<div class="col-sm-6 col-xs-12">
                          <div class="form-group">
													<input type="text" name="daterange" id="daterange" class="form-control input-sm " value="" placeholder="Select Date Range" />
                            </div>
												</div>
											</div>
                                                </div>	
                                        ...
                                        <div class="col-md-6 col-sm-12 col-xs-12 text-right">
                                            <button  type="button" onclick="data_search_user('changesearch')" class="btn btn-primary howler"  title="Search"><?php echo lang("search"); ?></button>
													...
													<?php /*if (checkAdminPermission('CancelledReservedList', 'add')) { ?>
														<button  type="button" class="btn btn-primary howler flt" title="ResendEmail" onclick="resendEmails();" title="ResendEmail"><?php echo lang("re_send_email"); ?></button>
													<?php } */?>
													<?php if (checkAdminPermission('CancelledReservedList', 'add')) { ?>
														<!-- <button type="button" class="btn btn-primary howler flt" title="ExportExcel" onclick="exportExcel();" title="ExportExcel"><?php echo lang("exportExcel"); ?></button> -->
													<?php } ?>
                                        </div>
                                    </div>
                                </form>
